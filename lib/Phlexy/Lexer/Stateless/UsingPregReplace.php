...
class UsingPregReplace implements \Phlexy\Lexer {
    ...
    public function __construct($compiledRegex, array $offsetToTokenMap, array $offsetToLengthMap) {
        $this->compiledRegex = '~(?:^|\G)' . substr($compiledRegex, 1, -1);
    ...
    public function lex($string) {
        ...
        $res = preg_replace_callback(
            ...
            function($matches) use($offsetToTokenMap, $offsetToLengthMap, &$tokens, &$line) {
                ...
                $realMatches = array();
                for ($j = 1, $length = $offsetToLengthMap[$i - 1]; $j < $length; ++$j) {
                ...
                if (!empty($realMatches)) {
                    $tokens[] = array($offsetToTokenMap[$i - 1], $line, $matches[0], $realMatches);
                } else {
                    $tokens[] = array($offsetToTokenMap[$i - 1], $line, $matches[0]);
                }
        ...
        if ('' !== $res) {
            throw new \Phlexy\LexingException(sprintf(
                'Unexpected character "%s" on line %d', $string[0], $line
            ));
        }
