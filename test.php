...
$sharedStringRegexes = array(
    ...
    '\$' . $labelRegex => T_VARIABLE, // order important
    '\$\{(?=' . $labelRegex . '[\[}])' => function(Stateful $lexer) {
        $lexer->pushState('LOOKING_FOR_VARNAME');

        return T_DOLLAR_OPEN_CURLY_BRACES;
    },
    '\$\{' => function(Stateful $lexer) { // order important
...
$lexerData = array(
    ...
    'IN_SCRIPTING' => array(
        ...
        // order dependent; can be made independent by including the negation of previous rule
        '->' => T_OBJECT_OPERATOR,
...
/*foreach ($lexerData as $state => $regexMap) {
    foreach ($regexMap as $regex => $action) {
        echo $regex, "\n";
        if (preg_match('~' . str_replace('~', '\~', $regex) . '~A', '')) {
...
$lexer = $factory->createLexer($lexerData, 'i');

// prevent token_get_all from lexing <?xml
